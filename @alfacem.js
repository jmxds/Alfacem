function processarCriacaoUsuarios() {
  ...
  const abaCriacao = spreadsheet.getSheetByName('CriacaoAlfa');
  
  if (!abaCriacao) {
    Logger.log('A aba "CriacaoAlfa" não foi encontrada.'); 
    return;
  }
  ...
  for (let i = 1; i < dados.length; i++) {
    ...
    if (nomeCompleto && dados[i][4] !== "Criado") {
      ...
      for (let j = nomePartes.length - 1; j > 0; j--) {
        email = `${nomePartes[0]}.${nomePartes[j]}@alfacem.com.br`.toLowerCase();
      ...
      if (!emailCriado) {
        for (let j = 1; j < nomePartes.length - 1; j++) {
          ...
          email = `${nomePartes[0]}.${inicialSobrenome}.${ultimoNomePadrao}@alfacem.com.br`.toLowerCase();
