function processarCriacaoUsuarios5() {
  const spreadsheet = SpreadsheetApp.getActiveSpreadsheet();
  const abaCriacao = spreadsheet.getSheetByName('Criacao Prof');
  
  if (!abaCriacao) {
    Logger.log('A aba "Criacao Prof" não foi encontrada.'); 
    return;
  }
  
  const GRUPO_TODOS_ID = 'todos@alfacem.com.br';
  const GRUPO_PROFESSORES_ID = 'todosprofessores@alfacem.com.br';
  ...
  for (let i = 1; i < dados.length; i++) {
    ...
    if (nomeCompleto && dados[i][4] !== "Criado") {
      ...
      for (let j = nomePartes.length - 1; j > 0; j--) {
        email = `${nomePartes[0]}.${nomePartes[j]}@prof.alfacem.com.br`.toLowerCase();
      ...
      if (!emailCriado) {
        for (let j = 1; j < nomePartes.length - 1; j++) {
          ...
          email = `${nomePartes[0]}.${inicialSobrenome}.${ultimoNomePadrao}@prof.alfacem.com.br`.toLowerCase();
      ...
      const message = criarUsuarioEAdicionarGrupos5(nomeCompleto, email, senhaPadrao, GRUPO_TODOS_ID, GRUPO_PROFESSORES_ID);
...
function criarUsuarioEAdicionarGrupos5(nomeCompleto, email, senhaPadrao, grupoTodosId, grupoProfessoresId) {
  try {
    const user = {
      primaryEmail: email,
      name: {
    ...
    const grupos = [grupoTodosId, grupoProfessoresId];
    ...
    grupos.forEach(grupoId => {
      try {
        ...
        Logger.log(`Erro ao adicionar usuário ao grupo ${grupoId}: ` + grupoError);
      }
    });
